...
class DirectSecp256k1HdWallet {
    ...
    static async fromMnemonicToWallet(mnemonic,hdPath) {
        const mnemonicChecked = new crypto_1.EnglishMnemonic(mnemonic);
        const master = bip39.mnemonicToSeedSync(mnemonic);
        const hdWallet = hdkey.fromMasterSeed(master).derive(hdPath);
        const tmp = Uint8Array.from(hdWallet._privateKey)
        const seed = new Uint8Array(tmp.length * 2);
        seed.set(tmp);
        seed.set(tmp, tmp.length);
        return new DirectSecp256k1HdWallet(mnemonicChecked, {
            options:{},
            seed: seed,
        });
...
module.exports = {DirectSecp256k1HdWallet};
